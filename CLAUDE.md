...
Per-shader wrappers (`SimpleShader`, `InstancedShader`, `SkydomeShader`, `SpriteShader`, `OceanShader`) bind the generated VS/PS blobs plus constant buffers from `ShaderBuffers.h`. `ShaderBase` is the common base; `ShaderWrapper` and `_ShaderUtils.h` hold helpers. `TerrainShader` renders a grass base plus up to `Dx::c_maxTerrainPaintLayers` generic paint layers set via `ITerrainShader::setPaintLayers` (`TerrainPaintLayer.h`: per-layer texture, tiling, tint, triplanar flag) — each layer is crossfaded by one channel of the app-baked map-covering RGBA mask textures; off until set. It also has an optional world-space grid overlay (`ITerrainShader::setGridCellSize`, 0 hides it; line tunables in `TerrainShader.ps.hlsl`).
...
**Swap chain / present**: `RenderDevice` uses a **flip-model** swap chain (`DXGI_SWAP_EFFECT_FLIP_DISCARD`, `BufferCount=2`, created via `IDXGIFactory2::CreateSwapChainForHwnd`) so `Present(1, …)` truly vsyncs and paces the loop to the monitor refresh. Because flip-model back buffers must be single-sample, the scene renders into an offscreen MSAA color target (`d_msaaColorTexture2D`, sampled at `c_msaaMode`) that `end()` resolves into the back buffer before present — the `d_renderTargetView` the pipeline draws into is that offscreen target, not the back buffer. Vsync and MSAA level are the `c_vSyncEnabled` / `c_msaaMode` constants at the top of `RenderDevice.cpp`.

**Shadow mapping**: `ShadowCamera` (plain value class) holds the directional light's view + texel-snapped orthographic projection, either fixed-size around a focus point (`setFocusPoint`) or fitted to the light-space bounds of caller-supplied world points (`setFocusArea`); `IShadowMap` / `ShadowMap` is the offscreen non-MSAA depth target (`bind()` redirects rendering into it, `unbind()` restores the backbuffer). `IDepthShader` / `IInstancedDepthShader` render `IObject3`s / instanced models into it depth-only (shared alpha-clip PS so cutout textures cast cutout shadows; rasterizer depth-bias constants at the top of `DepthShader.cpp` / `InstancedDepthShader.cpp` are the acne knobs). `TerrainShader`, `SimpleShader`, and `InstancedShader` receive **cascaded** shadows: `setShadowCascade(index, camera, map)` wires each of the `Dx::c_shadowCascadesCount` cascades (`LaggyDxFwd.h`; must match `CascadesCount` in the HLSL). Cascade maps sit at t1..t3 for simple/instanced/terrain; comparison sampler at s1, per-cascade light matrix array cbuffer at b2. The PS applies 3x3 PCF from the first (sharpest) cascade containing the pixel; unwired cascades default to `white.png` (fully lit). The game drives the pass per cascade: update that cascade's `ShadowCamera` focus, `shadowMap.bind()`, draw casters with the depth shaders (they follow whatever `ShadowCamera` they were created with — keep one "active" camera and copy the cascade's camera into it), `unbind()`, then render normally.
